...
with open(output_file, "w", encoding="utf-8") as f:
    ...
    for folder in found_folders:
        ...
        # Sadece verdiğin listedeki kanalları ekleyelim (Temiz liste)
        if channel_id in CHANNEL_DATA:
            ...
            if video_files:
                ...
                for video_file in video_files:
                    ...
                    # Logoyu sunucudan çek (yoksa boş bırak)
                    logo_url = LOGO_BASE + data["logo"] if data["logo"] else ""
                    
                    # GÖRÜNÜM: Kanal Adı (Saat:00 - Gün.Ay)
                    display_name = f"{data['name']} ({time_label}:00 - {bugun})"
